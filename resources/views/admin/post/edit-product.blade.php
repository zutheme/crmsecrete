				...
				<span class="dropup">
				    ...
				    <ul class="dropdown-menu">
				      @foreach($sel_cross_type as $row)
	                		<li><a href="{{ action('Admin\PostsController@create',['idposttype'=>$product[0]['id_post_type'],'idparent' => $idproduct,'idcrosstype' => $row['idcrosstype']] ) }}" class="btn btn-default btn-create-new">{{ $row['namecross'] }}</a></li>
						@endforeach 
				    </ul>
				  </span>
