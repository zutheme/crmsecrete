  ...
  <div id="invoice-POS">
    ...
    <div id="top">
      ...
      <div class="address"> 
        <p style="font-size: 0.7em !important;color: #000;line-height: 1.2em;"> 
            Địa chỉ   : Số 9, Đường D5, Khu biệt thự Saigon Pearl, 92 Nguyễn Hữu Cảnh, P.22, Q.Bình Thạnh, TP.HCM</br>
            Email     : allysfast@gmail.com</br>
            Điện thoại:0931536528</br>
        </p>
      </div>
    </div>
    ...
    <div id="bot">
		...
		<div style="width:100%;text-align:left;font-size:0.7em;font-weight:300;">
			...
			<table width='100%' style="text-align:left;font-size:0.7em;font-weight:300;">
				@foreach($rs_customerorder as $customer)
						@if($customer['cus'] == 1)
						 <tr width='100%'>
							<td width='30%'>Họ tên:</td><td width='70%'>{{ $customer['lastname'] }} {{ $customer['middlename'] }} {{$customer['firstname']}}</td>
						  </tr>
						  <tr>                
							<td  width='30%'>Điện thoại:</td><td width='70%'>{{ $customer['mobile'] }}</td>
						  </tr>
						  <tr>
							<td  width='30%'>Địa chỉ:</td><td width='70%'>{{ $customer['address'].','.$customer['nameward'].','.$customer['namedist'].','.$customer['namecitytown'].','.$customer['nameprovince'].','.$customer['namecountry'] }}</td>
						  </tr>
						@endif
					  @endforeach  
			</table>
		</div>
		<div id="legalcopy" style="border-top:1px dotted #000; text-align:center;font-size:0.7em;font-weight:300;">
