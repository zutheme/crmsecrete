	...
	<div class="col-12">
		<div class="card">
			<div class="card-body">
				...
				@endif
				<div align="right">
					<a class="btn btn-default btn-primary" href="{{ route('admin.attribute.create') }}">Thêm mới</a>
				</div>
				<div class="table-responsive m-t-40">
					<table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
						...
						<tbody>
						@if(isset($rs_attributes))
							@foreach($rs_attributes as $row)
								<tr>
									<td>{{ $row['idattribute'] }}</td>					
									<td>{{ $row['nameattribute'] }}</td>
									<td>{{ $row['nametype'] }}</td>
									<td class="btn-control"><a class="btn btn-primary btn-edit" href="{{ action('Admin\AttributeController@edit',$row['idattribute']) }}"><i class="fa fa-edit"></i></a></td>
									<td class="btn-control">
										 <form method="post" class="delete_form" action="{{action('Admin\AttributeController@destroy', $row['idattribute'])}}">
										  {{csrf_field()}}
